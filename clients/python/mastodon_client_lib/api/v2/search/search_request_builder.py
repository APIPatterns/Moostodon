...
search_result = lazy_import('mastodon_client_lib.models.search_result')
...
class SearchRequestBuilder():
    ...
    async def get(self,request_configuration: Optional[SearchRequestBuilderGetRequestConfiguration] = None) -> Optional[search_result.SearchResult]:
        request_info = self.to_get_request_information(
            request_configuration
        )
        if not self.request_adapter:
            raise Exception("Http core is null") 
        return await self.request_adapter.send_async(request_info, search_result.SearchResult, None)
